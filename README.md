...
.venv/bin/python main.py   # → http://0.0.0.0:9090
```
...
```bash
# 1) 토큰 발급
TOKEN=$(curl -s -X POST http://localhost:9090/auth/token \
...
| 포트 충돌 (`Address already in use`) | 다른 프로세스가 점유 중. `lsof -iTCP:9090 -sTCP:LISTEN`로 확인 후 `PORT` 변경 |
